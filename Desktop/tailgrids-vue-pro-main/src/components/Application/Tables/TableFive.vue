    ...
    <div class="container mx-auto">
      <div class="flex flex-wrap -mx-4">
        <div class="w-full px-4">
          <div
            class="max-w-full overflow-x-auto rounded-xl shadow-[0px_3px_8px_0px_rgba(0,0,0,0.08)] bg-white dark:bg-dark-2"
          >
            <table class="w-full table-auto">
              <thead>
                <tr class="bg-primary text-left">
                  <th
                    ...
                    :key="index"
                    :class="`py-4 px-4 first:pl-11 last:pr-11 text-base font-medium text-white ${header.styles}`"
                  >
                    {{ header.name }}
                  </th>
                </tr>
              ...
              <tbody>
                <tr v-for="(user, index) in users" :key="index">
                  <td class="px-4 py-5 border-t border-stroke dark:border-dark-3 pl-11">
                    <h5 class="text-base text-body-color dark:text-dark-6">
                      {{ user.name }}
                    </h5>
                  </td>
                  <td class="px-4 py-5 border-t border-stroke dark:border-dark-3">
                    <p class="text-base text-body-color dark:text-dark-6">{{ user.title }}</p>
                  </td>
                  <td class="px-4 py-5 border-t border-stroke dark:border-dark-3">
                    <p class="text-base text-body-color dark:text-dark-6">{{ user.email }}</p>
                  </td>
                  <td class="px-4 py-5 border-t border-stroke dark:border-dark-3">
                    <p class="text-base text-body-color dark:text-dark-6">{{ user.role }}</p>
                  </td>
                  <td class="px-4 py-5 text-right border-t border-stroke pr-11 dark:border-dark-3">
                    <div class="relative">
                      <select
                        class="text-body-color shadow-1 dark:bg-dark-3 dark:text-dark-6 dark:shadow-card appearance-none rounded-md bg-white py-[6px] pl-3 pr-8 text-sm outline-none"
                      >

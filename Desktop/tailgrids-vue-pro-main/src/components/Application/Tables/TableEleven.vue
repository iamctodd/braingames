  ...
  <section class="bg-gray-2 dark:bg-dark py-20 lg:py-[120px]">
    <div class="px-4 mx-auto lg:container">
      <div class="bg-white rounded-lg dark:bg-dark-2 shadow-card">
        <div
          class="items-center justify-between space-y-4 border-b border-stroke dark:border-dark-3 py-[18px] px-5 sm:flex sm:space-x-4 sm:space-y-0"
        >
          <div class="w-full md:max-w-[420px]">
            <div class="relative overflow-hidden rounded">
              <input
                ...
                class="h-[46px] w-full rounded-md dark:border-dark-3 border bg-transparent dark:text-dark-6 placeholder:opacity-60 border-stroke pl-5 pr-14 text-base text-body-color outline-none focus:border-primary"
              />
              <button
                class="absolute top-0 right-0 flex h-full w-[46px] items-center justify-center bg-primary text-white"
              >
                <svg
                  width="18"
                  height="18"
          ...
          <div>
            <button
              class="flex h-[46px] items-center justify-center whitespace-nowrap rounded-md bg-primary px-6 text-base font-medium text-white hover:bg-opacity-90"
            >
              <span class="mr-2">
                <svg
                  width="20"
                  ...
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clip-path="url(#clip0_1487_3014)">
                    <path
            ...
            <thead>
              <tr class="border-b border-stroke dark:border-dark-3">
                <th
                  ...
                  :key="index"
                  :class="`px-4 pt-9 pb-6 first:pl-8 align-top ${header.styles}`"
                >
                  <div v-if="index === 0" class="relative mt-1">
                    <input
                      type="checkbox"
                      :id="'checkbox-' + index + 1"
                      name="tableCheckbox"
                      class="sr-only tableCheckbox-2"
                    />
                    <label
                      :for="'checkbox-' + index + 1"
                      class="flex h-[18px] w-[18px] cursor-pointer items-center justify-center rounded border border-stroke dark:border-dark-4"
                    >
                      <span class="opacity-0 icon text-whit">
                  ...
                  <div v-if="index !== 0 && index !== headers.length - 1">
                    <div class="mb-[10px] flex items-center">
                      <p class="mr-1 text-base font-medium text-body-color dark:text-dark-6">
                        {{ header.name }}
                      </p>
                      <div
                        class="inline-flex flex-col space-y-[2px] text-body-color dark:text-dark-6"
                      >
                    ...
                    <div class="relative">
                      <input
                        ...
                        class="h-[34px] w-full rounded border border-stroke pr-3 pl-8 text-sm text-body-color outline-none dark:border-dark-3 dark:text-dark-6 bg-transparent"
                      />
                      <span
                        v-if="header.searchIcon"
                        class="absolute -translate-y-1/2 top-1/2 left-3 text-body-color dark:text-dark-6"
                      >
                        <svg
                          width="14"
                          height="14"
                      ...
                      <span
                        v-if="header.arrowIcon"
                        class="absolute -translate-y-1/2 text-body-color dark:text-dark-6 right-3 top-1/2"
                      >
                        <svg
                          width="10"
                          height="5"
                      ...
                      <span
                        v-if="header.calendarIcon"
                        class="absolute -translate-y-1/2 text-body-color dark:text-dark-6 top-1/2 left-3"
                      >
                        <svg
                          width="14"
                          height="14"
                  ...
                  <div v-if="index === headers.length - 1 && null"></div>
                </th>
              </tr>
            </thead>
            <tbody>
              <tr
                ...
                :key="index"
                class="border-b last:border-none border-stroke dark:border-dark-3 hover:bg-primary/5"
              >
                <td class="py-5 pl-8">
                  <div class="relative">
                    <input
                    ...
                    <label
                      ...
                      class="flex h-[18px] w-[18px] cursor-pointer items-center justify-center rounded border border-stroke dark:border-dark-4"
                    >
                      <span class="text-white opacity-0 icon">
                        <svg width="10" height="10" viewBox="0 0 10 10">
                          <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                ...
                <td class="py-5 px-4">
                  <p class="text-base font-medium text-primary">{{ user.name }}</p>
                  <p class="text-base text-body-color dark:text-dark-6">{{ user.id }}</p>
                </td>
                <td class="py-5 px-4">
                  <p class="text-base text-body-color dark:text-dark-6">{{ user.position }}</p>
                  <p class="text-base text-body-color dark:text-dark-6">{{ user.duration }}</p>
                </td>
                <td class="py-5 px-4">
                  <p class="text-base text-body-color dark:text-dark-6">{{ user.birthDate }}</p>
                </td>
                <td class="py-5 px-4">
                  <p class="text-base text-body-color dark:text-dark-6">{{ user.email }}</p>
                  <p class="text-base text-body-color dark:text-dark-6">{{ user.phone }}</p>
                </td>
                <td class="py-5 px-4">
                  <p class="text-base text-body-color dark:text-dark-6">{{ user.address }}</p>
                </td>
                <td class="py-5 px-4">
                  <p class="text-base text-body-color dark:text-dark-6">{{ user.status }}</p>
                </td>
                <td class="py-5 pl-4 pr-8">
                  <div class="relative">
                    ...
                    <div
                      ...
                      class="absolute right-0 top-full z-40 w-[200px] space-y-1 rounded bg-white p-2 shadow-card dark:bg-dark border-stroke dark:border-dark-3 border"
                    >
                      <button
                        @click.stop="closeDropdown(index)"

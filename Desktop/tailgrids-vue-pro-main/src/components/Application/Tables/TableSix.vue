    ...
    <div class="container mx-auto">
      <div class="-mx-4 flex flex-wrap justify-center">
        <div class="w-full px-4 lg:w-10/12">
          <div
            class="p-6 bg-white rounded-[10px] dark:bg-dark-2 shadow-[0px_1px_4px_0px_rgba(0,0,0,0.15)] lg:p-9"
          >
            <div class="max-w-full overflow-x-auto">
              <table class="w-full table-auto">
                <thead>
                  <tr class="text-left rounded bg-gray-2 dark:bg-dark-3">
                    <th
                      v-for="(header, index) in headers"
                      :key="index"
                      :class="`py-4 px-4 first:pl-9 xl:first:pl-11 last:pr-9 xl:last:pr-11 text-base font-medium uppercase text-dark dark:text-white ${header.styles}`"
                    >
                      {{ header.name }}
                    </th>
                  </tr>
                ...
                <tbody>
                  <tr v-for="(user, index) in users" :key="index">
                    <td class="px-4 py-5 border-b border-stroke dark:border-dark-3 pl-9 xl:pl-11">
                      <h5 class="text-base font-medium text-dark dark:text-white">
                        {{ user.name }}
                      </h5>
                      <p class="text-sm dark:text-dark-6 text-body-color">{{ user.rate }}</p>
                    </td>
                    <td class="px-4 py-5 border-b border-stroke dark:border-dark-3">
                      <p class="text-base text-dark dark:text-dark-6">{{ user.invoiceDate }}</p>
                    </td>
                    <td class="px-4 py-5 border-b border-stroke dark:border-dark-3">
                      <p
                        v-if="user.status === 'paid'"
                        class="inline-flex px-3 py-1 text-sm font-medium rounded-full bg-green-light-6 text-green-dark"
                      >
                        Paid
                      </p>
                      <p
                        v-if="user.status === 'unpaid'"
                        class="inline-flex px-3 py-1 text-sm font-medium rounded-full bg-red-light-5 text-red-dark"
                      >
                        Unpaid
                      </p>
                      <p
                        v-if="user.status === 'pending'"
                        class="inline-flex px-3 py-1 text-sm font-medium rounded-full bg-yellow-light-4 text-yellow-dark-2"
                      >
                        Pending
                      </p>
                    </td>
                    <td class="px-4 py-5 border-b border-stroke dark:border-dark-3">
                      <div class="flex items-center space-x-4">
                        <button class="text-body-color hover:text-primary dark:text-dark-6">
                          <svg
                            width="22"
                            height="22"
                            viewBox="0 0 22 22"

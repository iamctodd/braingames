...
<template>
  <section class="bg-gray-2 dark:bg-dark py-20 lg:py-[120px]">
    <div class="mx-auto px-4 md:container">
      <div class="px-6 py-4 mb-5 bg-white rounded-md dark:bg-dark-2 shadow-2">
        <div class="relative">
          <input
            type="text"
            placeholder="Search for user, email address..."
            class="block w-full text-base bg-transparent outline-none placeholder:opacity-60 px-7 text-body-color dark:text-dark-6"
          />
          <span class="absolute left-0 -translate-y-1/2 text-body-color dark:text-dark-6 top-1/2">
            <svg
              width="18"
              height="18"
              viewBox="0 0 18 18"
          ...
          <button
            class="absolute right-0 -translate-y-1/2 text-body-color dark:text-dark-6 top-1/2"
          >
            <svg
              width="18"
              height="18"
      ...
      <div class="bg-white border rounded-md border-stroke dark:bg-dark-2 dark:border-dark-3">
          ...
          <table class="w-full table-auto">
            <thead>
              <tr class="border-b border-stroke dark:border-dark-3">
                <th
                  ...
                  :key="index"
                  :class="`py-6 px-4 first:pl-10 last:pr-10 ${header.styles}`"
                >
                  <div v-if="header.checkbox" class="relative">
                    <input
                      type="checkbox"
                      :id="'checkbox-' + index + 1"
                      name="tableCheckbox"
                    ...
                    <label
                      ...
                      class="flex items-center text-base font-medium cursor-pointer text-body-color dark:text-dark-6"
                    >
                      <span
                        class="icon-box mr-3 flex h-4 w-4 items-center justify-center rounded-[3px] border-[.5px] border-stroke bg-white text-white dark:border-dark-3 dark:bg-dark"
                      >
                  ...
                  <p v-else class="text-base font-medium text-body-color dark:text-dark-6">
                    {{ header.name }}
                  </p>
                </th>
              </tr>
            ...
            <tbody>
              <tr
                ...
                :key="index"
                class="hover:bg-gray-2 dark:hover:bg-dark-3"
              >
                <td class="py-4 pl-10 pr-4">
                  <div class="relative">
                    <input
                      type="checkbox"
                      :id="'checkbox-' + index"
                    ...
                    <label
                      :for="'checkbox-' + index"
                      class="flex items-center text-base font-normal cursor-pointer text-body-color dark:text-dark-6"
                    >
                      <span
                ...
                <td class="p-4">
                  <p class="text-base text-body-color dark:text-dark-6">{{ user.subject }}</p>
                </td>
                <td class="py-4 pl-4 pr-10">
                  <p class="text-base text-body-color dark:text-dark-6 text-right">
                    {{ user.date }}
                  </p>
                </td>
              </tr>
        ...
        <div
          class="flex items-center justify-between p-5 mt-2 border-t sm:px-6 border-stroke dark:border-dark-3"
        >
          <p class="text-base text-left text-dark dark:text-white">1-5 of 29</p>
          <div class="flex items-center justify-end space-x-3">
            <button
              class="flex h-[30px] w-[30px] items-center justify-center rounded border border-stroke bg-[#FAFBFF] text-body-color dark:border-dark-3 dark:bg-dark dark:text-dark-6 hover:border-primary hover:bg-primary hover:text-white"
            >
              <svg
                width="18"
                height="18"

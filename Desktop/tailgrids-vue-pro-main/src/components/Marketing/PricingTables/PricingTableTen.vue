    ...
    <div class="container mx-auto">
      <div class="-mx-4 flex flex-wrap">
        <div class="w-full px-4">
          <div class="mx-auto mb-[60px] max-w-[510px] text-center">
            <span class="text-primary mb-2 block text-lg font-semibold"> Our Pricing Plans </span>
            <h2
      ...
      <div class="mb-10 text-center">
        <div class="flex items-center justify-center">
          <button
            class="border-b-[3px] py-4 px-6 text-lg font-semibold"
            :class="{
              'text-primary border-primary': billingCycle === 'monthly',
              'text-dark dark:text-white border-[#E9EEFF]': billingCycle === 'yearly'
            }"
          ...
          <button
            class="relative border-b-[3px] py-4 px-6 text-lg font-semibold"
            :class="{
              'text-primary border-primary': billingCycle === 'yearly',
              'text-dark dark:text-white border-[#E9EEFF]': billingCycle === 'monthly'
            }"
            @click="toggleBillingCycle"
          >
            Yearly
            <span
              class="absolute top-0 -right-14 whitespace-nowrap rounded-full bg-[#F47070] py-1 px-[10px] text-[10px] leading-[15px] font-semibold text-white"
            >
              Save 40%
            </span>
          </button>
      ...
      <div class="-mx-4 flex flex-wrap justify-center">
        <template v-for="(plan, index) in pricingPlans" :key="index">
          <div class="w-full px-4 md:w-1/2 lg:w-1/3">
            <div
              class="border-stroke dark:border-dark-3 relative z-10 mb-10 rounded-[15px] border-2 bg-white dark:bg-dark-2 py-11 px-8 sm:px-11 md:px-9 lg:px-8 xl:px-11"
            >
              <span class="absolute right-6 top-6 -z-10" v-html="plan.icon"> </span>

              <div>
                <h3
                  class="mb-2 text-xl font-semibold text-dark dark:text-white sm:text-2xl lg:text-xl xl:text-2xl"
                >
                  {{ plan.type }}
                </h3>
                <p class="text-body-color dark:text-dark-6 mb-8 max-w-[185px] text-sm">
                  {{ plan.subtitle }}
                </p>
              </div>

              <div class="mb-9 rounded-[10px] bg-[#F1F4FF] dark:bg-dark py-[18px] text-center">
                <h2
                  class="text-3xl font-bold text-dark dark:text-white sm:text-4xl lg:text-3xl xl:leading-[46px] xl:text-[38px]"
                >
                  <sup class="text-xl font-medium text-body-color dark:text-dark-6">$</sup>
                  <span v-text="billingCycle === 'yearly' ? plan.priceYearly : plan.price"> </span>
                  <span
                    class="text-body-color text-base font-medium dark:text-dark-6"
                    v-text="billingCycle === 'yearly' ? '/year' : '/month'"
                  ></span>
                </h2>
              </div>
              <h5 class="text-dark dark:text-white mb-6 text-lg font-medium">Plan Includes:</h5>

              <div class="mb-9 space-y-[14px]">
                <template v-for="(item, index) in plan.features" :key="index">
                  <p class="text-body-color dark:text-dark-6 flex items-center gap-3 text-base">
                    <span>
                      <svg
                        width="20"
                        height="20"
              ...
              <a
                href="/#"
                class="flex w-full items-center justify-center rounded-md border p-3 text-base font-medium capitalize"
                :class="{
                  ' border-transparent bg-primary text-white hover:bg-blue-dark': plan.popular,
                  'border-primary text-primary hover:bg-primary hover:text-white ': !plan.popular
                }"
              >
                {{ plan.button.text }}
              </a>

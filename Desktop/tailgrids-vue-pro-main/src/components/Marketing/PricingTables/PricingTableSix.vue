<script setup lang="ts">
import { ref } from 'vue'

const pricingPlans = ref([
  {
    popular: true,
    type: 'Free Lite',
    title: `It's totally free`,
    price: '0$',
    button: {
      text: 'Current Plan'
    },
    icon: `<svg
                      width="34"
                      height="34"
                      viewBox="0 0 34 34"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M32.407 16.0971C31.8758 16.0971 31.4508 16.5221 31.4508 17.0533V27.4127C31.4508 29.7502 29.7508 31.5564 27.5727 31.5564H6.42891C4.25078 31.5564 2.55078 29.7502 2.55078 27.4127V17.0002C2.55078 16.4689 2.12578 16.0439 1.59453 16.0439C1.06328 16.0439 0.691406 16.4689 0.691406 17.0002V27.3596C0.691406 30.6533 3.29453 33.3627 6.42891 33.3627H27.5727C30.7602 33.3627 33.3102 30.6533 33.3102 27.3596V17.0002C33.3633 16.4689 32.9383 16.0971 32.407 16.0971Z"
                        fill="#3056D3"
                      />
                      <path
                        d="M15.2477 22.1533C15.7258 22.6314 16.3633 22.8439 17.0008 22.8439C17.6383 22.8439 18.2227 22.6314 18.7008 22.1533L23.2695 17.6377C23.6414 17.2658 23.6414 16.6814 23.2695 16.3096C22.8977 15.9377 22.3133 15.9377 21.9414 16.3096L17.957 20.2939V1.59395C17.957 1.0627 17.532 0.637695 17.0008 0.637695C16.4695 0.637695 16.0977 1.0627 16.0977 1.59395V20.3471L12.0602 16.3096C11.6883 15.9377 11.1039 15.9377 10.732 16.3096C10.3602 16.6814 10.3602 17.2658 10.732 17.6377L15.2477 22.1533Z"
                        fill="#3056D3"
                      />
                    </svg>`,
    features: [
      { isChecked: true, text: 'Up to 1 User' },
      { isChecked: true, text: 'All UI components' },
      { isChecked: true, text: 'Lifetime access' },
      { isChecked: true, text: 'Free Updates' },
      { isChecked: true, text: 'Up to 1 User' },
      { isChecked: false, text: 'Community Support' },
      { isChecked: false, text: 'Downloadable Files' }
    ]
  },
  {
    type: 'Starter',
    title: `Single Site`,
    price: '29$',
    discountPrice: '39$',
    button: {
      text: 'Get the plan'
    },
    icon: `<svg width="34" height="34" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                           <path d="M27.2001 20.3469L24.8625 19.0719V15.3C24.8625 6.90626 21.1969 3.45314 17.6375 1.16876C17.2657 0.903137 16.7344 0.903137 16.3625 1.16876C12.8032 3.40001 9.13755 6.85314 9.13755 15.3531V19.125L6.80005 20.4C5.31255 21.1969 4.40942 22.7375 4.40942 24.4375V28.475C4.40942 28.9 4.62192 29.2719 4.9938 29.4844C5.2063 29.5906 5.4188 29.6438 5.6313 29.6438C5.8438 29.6438 6.00317 29.5906 6.21567 29.4844L9.19067 27.8906V27.9969C9.19067 28.6344 9.72192 29.2188 10.4125 29.2188H11.2094V31.875C11.2094 32.5125 11.7407 33.0969 12.4313 33.0969H21.7282C22.3657 33.0969 22.9501 32.5656 22.9501 31.875V29.1656H23.8C24.4375 29.1656 25.0219 28.6344 25.0219 27.9438V27.8375L27.9969 29.4313C28.1563 29.5375 28.3688 29.5906 28.5813 29.5906C28.7938 29.5906 29.0063 29.5375 29.2188 29.4313C29.5907 29.2188 29.8032 28.8469 29.8032 28.4219V24.3844C29.6438 22.6844 28.6875 21.1438 27.2001 20.3469ZM11.5282 15.3C11.5282 8.50001 14.0782 5.52501 17 3.55939C19.9219 5.57814 22.4719 8.55314 22.4719 15.3V26.775H18.1688V20.4C18.1688 19.7625 17.6375 19.1781 16.9469 19.1781C16.3094 19.1781 15.725 19.7094 15.725 20.4V26.7219H11.4219V15.3H11.5282ZM6.74692 26.4031V24.3844C6.74692 23.5875 7.17192 22.8438 7.91567 22.4188L9.13755 21.7813V25.1281L6.74692 26.4031ZM20.4532 30.7063H13.5469V29.1656H20.4532V30.7063ZM27.2532 26.4031L24.9157 25.1281V21.7813L26.1376 22.4188C26.8282 22.7906 27.3063 23.5344 27.3063 24.3844V26.4031H27.2532Z" fill="#3758F9"></path>
                           <path d="M17 16.0437C18.8063 16.0437 20.2938 14.5562 20.2938 12.75C20.2938 10.9437 18.8063 9.45624 17 9.45624C15.1938 9.45624 13.7063 10.9437 13.7063 12.75C13.7063 14.5562 15.1938 16.0437 17 16.0437ZM17 11.8469C17.4782 11.8469 17.9032 12.2719 17.9032 12.75C17.9032 13.2281 17.4782 13.6531 17 13.6531C16.5219 13.6531 16.0969 13.2281 16.0969 12.75C16.0969 12.2719 16.5219 11.8469 17 11.8469Z" fill="#3758F9"></path>
                        </svg>`,
    features: [
      { isChecked: true, text: 'Up to 1 User' },
      { isChecked: true, text: 'All UI components' },
      { isChecked: true, text: 'Lifetime access' },
      { isChecked: true, text: 'Free Updates' },
      { isChecked: true, text: 'Up to 1 User' },
      { isChecked: true, text: 'Community Support' },
      { isChecked: false, text: 'Downloadable Files' }
    ]
  },
  {
    type: 'Business',
    title: `Unlimited sites`,
    price: '59$',
    discountPrice: '99$',
    button: {
      text: 'Get the plan'
    },
    icon: `<svg width="34" height="34" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                           <path d="M13.3359 11.9003C13.3359 13.9722 15.0359 15.6722 17.1078 15.6722C19.1797 15.6722 20.8797 13.9722 20.8797 11.9003C20.8797 9.82842 19.1797 8.12842 17.1078 8.12842C15.0359 8.18154 13.3359 9.88154 13.3359 11.9003ZM18.9672 11.9003C18.9672 12.9628 18.1172 13.8128 17.0547 13.8128C15.9922 13.8128 15.1422 12.9628 15.1422 11.9003C15.1422 10.8378 15.9922 9.98779 17.0547 9.98779C18.1172 9.98779 18.9672 10.8909 18.9672 11.9003Z" fill="#3056D3"></path>
                           <path d="M33.0448 21.7815L30.5479 19.2847C30.2823 19.019 29.8573 18.9128 29.4854 19.0722L26.7229 20.294C26.2448 20.5065 26.0323 21.0378 26.2448 21.5159C26.4573 21.994 26.9885 22.2065 27.4666 21.994L28.5823 21.5159C27.5729 24.119 25.7135 26.244 23.376 27.6253V22.3128C23.376 19.6034 21.4104 17.4253 18.9135 17.4253H14.876C12.4323 17.4253 10.4135 19.6034 10.4135 22.3128V27.4659C8.0229 25.9784 6.16352 23.6409 5.2604 20.8253C5.10102 20.3472 4.56977 20.0815 4.09165 20.2409C3.61352 20.4003 3.3479 20.9315 3.50727 21.4097C5.41977 27.3065 10.8916 31.2378 17.0541 31.2378C23.1104 31.2378 28.4229 27.4659 30.4948 21.7815L31.7166 23.0034C31.876 23.1628 32.1416 23.269 32.3541 23.269C32.5666 23.269 32.8323 23.1628 32.9916 23.0034C33.4166 22.7378 33.4166 22.1534 33.0448 21.7815ZM12.326 28.4753V22.3659C12.326 20.719 13.4948 19.3378 14.9291 19.3378H16.6823L15.7791 21.8878C15.6729 22.2065 15.6729 22.5253 15.7791 22.7909L16.5229 25.2347C16.6823 25.7128 17.3198 25.7128 17.4791 25.2347L18.2229 22.7909C18.3291 22.4722 18.3291 22.1534 18.2229 21.8878L17.3198 19.3378H19.0729C20.5073 19.3378 21.676 20.6659 21.676 22.3659V28.5815C20.2416 29.1128 18.701 29.4315 17.1073 29.4315C15.4073 29.4315 13.8135 29.1128 12.326 28.4753Z" fill="#3056D3"></path>
                           <path d="M4.51641 16.2564L7.49141 14.6627C7.96953 14.3971 8.12891 13.8658 7.86328 13.3877C7.59766 12.9627 7.06641 12.7502 6.58828 13.0158L5.10078 13.8127C6.53516 8.39395 11.4227 4.56895 17.107 4.56895C22.8445 4.56895 27.7852 8.44707 29.2195 14.0252C29.3258 14.5033 29.857 14.8221 30.3352 14.7158C30.8133 14.6096 31.132 14.0783 31.0258 13.6002C29.432 7.2252 23.6945 2.7627 17.107 2.7627C10.6258 2.7627 4.99453 7.11895 3.29453 13.3877L2.23203 12.3252C1.86016 11.9533 1.27578 11.9533 0.903906 12.3252C0.532031 12.6971 0.532031 13.2814 0.903906 13.6533L3.40078 16.1502C3.56016 16.3096 3.82578 16.4158 4.03828 16.4158C4.25078 16.3627 4.35703 16.3096 4.51641 16.2564Z" fill="#3056D3"></path>
                        </svg>`,
    features: [
      { isChecked: true, text: 'Up to 1 User' },
      { isChecked: true, text: 'All UI components' },
      { isChecked: true, text: 'Lifetime access' },
      { isChecked: true, text: 'Free Updates' },
      { isChecked: true, text: 'Up to 1 User' },
      { isChecked: true, text: 'Community Support' },
      { isChecked: true, text: 'Downloadable Files' }
    ]
  },
  {
    type: 'Extended',
    title: `For paying users`,
    price: '189$',
    discountPrice: '259$',
    button: {
      text: 'Get the plan'
    },
    icon: `<svg width="34" height="34" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                           <path d="M19.7098 25.606C17.5848 25.606 15.8848 27.3591 15.8848 29.4841C15.8848 31.6091 17.5848 33.3622 19.7098 33.3622C21.8348 33.3622 23.5348 31.6091 23.5348 29.4841C23.5348 27.3591 21.8348 25.606 19.7098 25.606ZM19.7098 31.5028C18.5941 31.5028 17.7441 30.5997 17.7441 29.4841C17.7441 28.3685 18.6473 27.4653 19.7098 27.4653C20.8254 27.4653 21.6754 28.3685 21.6754 29.4841C21.6754 30.5997 20.8254 31.5028 19.7098 31.5028Z" fill="#3056D3"></path>
                           <path d="M9.50859 25.606C7.38359 25.606 5.68359 27.3591 5.68359 29.4841C5.68359 31.6091 7.38359 33.3622 9.50859 33.3622C11.6336 33.3622 13.3336 31.6091 13.3336 29.4841C13.3336 27.3591 11.6336 25.606 9.50859 25.606ZM9.50859 31.5028C8.39297 31.5028 7.54297 30.5997 7.54297 29.4841C7.54297 28.3685 8.44609 27.4653 9.50859 27.4653C10.6242 27.4653 11.4742 28.3685 11.4742 29.4841C11.4742 30.5997 10.6242 31.5028 9.50859 31.5028Z" fill="#3056D3"></path>
                           <path d="M31.0251 0.69043H28.1563C26.9876 0.69043 25.9782 1.54043 25.8188 2.70918L24.9157 9.13731H22.1532V4.8873C22.1532 4.09043 21.5157 3.45293 20.7188 3.45293H16.522C16.4688 2.65605 15.8845 2.07168 15.1407 2.07168H10.5188C9.72196 2.07168 9.08447 2.76231 9.08447 3.6123V4.8873H4.88759C4.09072 4.8873 3.45322 5.47168 3.45322 6.26856V9.13731C3.02822 9.19043 2.60322 9.34981 2.33759 9.72168C2.07197 10.0936 1.96572 10.5717 2.07197 10.9967V11.0498L5.36572 21.0904C5.52509 21.7279 6.16259 22.2061 6.85322 22.2061H21.6751C23.5345 22.2061 25.1282 20.8248 25.3938 18.9654L27.6782 2.97481C27.7313 2.76231 27.8907 2.54981 28.1563 2.54981H31.0251C31.5563 2.54981 31.9813 2.1248 31.9813 1.59355C31.9813 1.0623 31.5563 0.69043 31.0251 0.69043ZM20.2938 9.08418H16.522V5.3123H20.2938V9.08418ZM10.9438 3.93106H14.7157V4.8873V9.08418H10.9438V6.26856V3.93106ZM5.31259 6.74668H9.08447V9.08418H5.31259V6.74668ZM23.5345 18.6998C23.4282 19.6029 22.5782 20.3467 21.6751 20.3467H7.11884L4.03759 10.9436H9.08447H10.9438H14.7157H16.5751H22.2063H24.7032L23.5345 18.6998Z" fill="#3056D3"></path>
                        </svg>`,
    features: [
      { isChecked: true, text: 'Up to 1 User' },
      { isChecked: true, text: 'All UI components' },
      { isChecked: true, text: 'Lifetime access' },
      { isChecked: true, text: 'Free Updates' },
      { isChecked: true, text: 'Up to 1 User' },
      { isChecked: true, text: 'Community Support' },
      { isChecked: true, text: 'Downloadable Files' }
    ]
  }
])
</script>

<template>
  <!-- ====== Pricing Section Start -->
  <section
    class="relative z-20 overflow-hidden bg-white dark:bg-dark pt-20 pb-12 lg:pt-[120px] lg:pb-[90px]"
  >
    <div class="container mx-auto">
      <div class="flex flex-wrap -mx-4">
        <div class="w-full px-4">
          <div class="mx-auto mb-[60px] max-w-[600px] text-center">
            <span class="block mb-2 text-lg font-semibold text-primary"> Our Pricing Plans </span>
            <h2
              class="mb-4 text-3xl leading-[1.208] font-bold text-dark dark:text-white sm:text-4xl md:text-[40px]"
            >
              Ready to start with tailGrids?
            </h2>
            <p class="mx-auto max-w-[510px] text-base text-body-color dark:text-dark-6">
              There are many variations of passages of Lorem Ipsum available but the majority have
              suffered alteration in some form.
            </p>
          </div>
        </div>
      </div>

      <div class="flex flex-wrap justify-center -mx-4">
        <template v-for="(plan, index) in pricingPlans" :key="index">
          <div class="w-full px-4 md:w-1/2 xl:w-1/4">
            <div
              class="p-8 bg-white border shadow-pricing-4 rounded-xl border-stroke dark:border-dark-3 dark:bg-dark-2"
            >
              <div class="pb-8 border-b border-stroke dark:border-dark-3">
                <div class="mb-[22px] flex justify-between">
                  <div>
                    <h3 class="text-[22px] leading-7 mb-1 font-semibold text-dark dark:text-white">
                      {{ plan.type }}
                    </h3>
                    <p class="text-base text-body-color dark:text-dark-6">{{ plan.title }}</p>
                  </div>
                  <div v-html="plan.icon"></div>
                </div>

                <h4 class="mb-1 text-[38px] leading-[46px] font-bold text-dark dark:text-white">
                  {{ plan.price }}
                </h4>
                <p class="mb-5 text-base text-body-color dark:text-dark-6">One time Payment</p>
                <button
                  :class="{
                    'flex w-full items-center justify-center rounded-md bg-dark py-[10px] text-base font-medium text-white hover:bg-body-color':
                      plan.popular,
                    'flex w-full items-center justify-center rounded-md bg-primary py-[10px] text-base font-medium text-white hover:bg-blue-dark':
                      !plan.popular
                  }"
                >
                  {{ plan.button.text }}
                </button>
              </div>

              <div class="pt-8">
                <div class="space-y-[14px]">
                  <template v-for="(item, index) in plan.features" :key="index">
                    <p class="flex items-center text-base text-dark dark:text-white">
                      <span class="pr-[10px]" v-if="item.isChecked">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <g clip-path="url(#clip0_1676_331)">
                            <path
                              d="M10 0.5625C4.78125 0.5625 0.5625 4.78125 0.5625 10C0.5625 15.2188 4.78125 19.4688 10 19.4688C15.2188 19.4688 19.4688 15.2188 19.4688 10C19.4688 4.78125 15.2188 0.5625 10 0.5625ZM10 18.0625C5.5625 18.0625 1.96875 14.4375 1.96875 10C1.96875 5.5625 5.5625 1.96875 10 1.96875C14.4375 1.96875 18.0625 5.59375 18.0625 10.0312C18.0625 14.4375 14.4375 18.0625 10 18.0625Z"
                              fill="#22AD5C"
                            />
                            <path
                              d="M12.6874 7.09374L8.96868 10.7187L7.28118 9.06249C6.99993 8.78124 6.56243 8.81249 6.28118 9.06249C5.99993 9.34374 6.03118 9.78124 6.28118 10.0625L8.28118 12C8.46868 12.1875 8.71868 12.2812 8.96868 12.2812C9.21868 12.2812 9.46868 12.1875 9.65618 12L13.6874 8.12499C13.9687 7.84374 13.9687 7.40624 13.6874 7.12499C13.4062 6.84374 12.9687 6.84374 12.6874 7.09374Z"
                              fill="#22AD5C"
                            />
                          </g>
                          <defs>
                            <clipPath id="clip0_1676_331">
                              <rect width="20" height="20" fill="white" />
                            </clipPath>
                          </defs>
                        </svg>
                      </span>

                      <span class="pr-[10px]" v-if="!item.isChecked">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <g clip-path="url(#clip0_1676_359)">
                            <path
                              d="M10 0.5625C4.78125 0.5625 0.5625 4.78125 0.5625 10C0.5625 15.2188 4.78125 19.4688 10 19.4688C15.2188 19.4688 19.4688 15.2188 19.4688 10C19.4688 4.78125 15.2188 0.5625 10 0.5625ZM10 18.0625C5.5625 18.0625 1.96875 14.4375 1.96875 10C1.96875 5.5625 5.5625 1.96875 10 1.96875C14.4375 1.96875 18.0625 5.59375 18.0625 10.0312C18.0625 14.4375 14.4375 18.0625 10 18.0625Z"
                              fill="#F56060"
                            />
                            <path
                              d="M12.875 7.09375C12.5938 6.8125 12.1563 6.8125 11.875 7.09375L10 9L8.09375 7.09375C7.8125 6.8125 7.375 6.8125 7.09375 7.09375C6.8125 7.375 6.8125 7.8125 7.09375 8.09375L9 10L7.09375 11.9062C6.8125 12.1875 6.8125 12.625 7.09375 12.9062C7.21875 13.0312 7.40625 13.125 7.59375 13.125C7.78125 13.125 7.96875 13.0625 8.09375 12.9062L10 11L11.9062 12.9062C12.0312 13.0312 12.2188 13.125 12.4063 13.125C12.5938 13.125 12.7812 13.0625 12.9062 12.9062C13.1875 12.625 13.1875 12.1875 12.9062 11.9062L11 10L12.9062 8.09375C13.1562 7.8125 13.1563 7.375 12.875 7.09375Z"
                              fill="#F56060"
                            />
                          </g>
                          <defs>
                            <clipPath id="clip0_1676_359">
                              <rect width="20" height="20" fill="white" />
                            </clipPath>
                          </defs>
                        </svg>
                      </span>
                      {{ item.text }}
                    </p>
                  </template>
                </div>
              </div>
            </div>
          </div>
        </template>
      </div>
    </div>
  </section>
  <!-- ====== Pricing Section End -->
</template>

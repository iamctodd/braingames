...
const avatarItems = ref([
  ...
  {
    image: 'https://cdn.tailgrids.com/2.0/image/dashboard/images/avatar/image-05.jpg',
    size: '80px',
    bulletSize: '18px'
  }
])
  ...
  <section class="bg-white dark:bg-dark py-[75px]">
    <div class="mx-auto px-4 sm:container">
      <div class="flex items-end justify-center space-x-2 sm:space-x-5">
        <template v-for="(item, index) in avatarItems" :key="index">
          <div class="relative rounded-full" :style="{ width: item.size, height: item.size }">
            <img
              :src="item.image"
              alt="avatar"
            ...
            />
            <span
               class="absolute top-0 right-0 block rounded-full border border-white dark:border-dark bg-[#219653]"
               :style="{ width: item.bulletSize, height: item.bulletSize }"
               ></span>
          </div>
        </template>
      </div>

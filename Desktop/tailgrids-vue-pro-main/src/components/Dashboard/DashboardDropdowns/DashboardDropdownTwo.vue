...
const dropdownItems = ref([
  {
    ...
    name: 'Mahmudul added',
    actionText: 'to his favorite list',
    textForAction: 'Leather Belt Steve Madden',
    time: '12 min ago.',
  ...
  },
  {
    ...
    href: 'javascript:void(0)',
    name: 'Labonno',
    actionText: 'leave her comment to Dressni',
    textForAction: 'Breathable Female Dress',
    time: '25 min ago.',
    actionDropdownVisible: false,
    actionItems: [
      { label: ' Mark As Unread' },
  ...
  },
  {
    ...
    href: 'javascript:void(0)',
    name: 'Tahmina',
    actionText: 'announce to 50% discoun',
    textForAction: 'New Exclusive Long Kurti',
    time: '45 min ago.',
    actionDropdownVisible: false,
    actionItems: [
      { label: ' Mark As Unread' },
    ...
    <div class="container">
      <div class="flex items-center justify-center">
        <div class="relative">
          ...
          >
            <span class="z-1 absolute -top-0.5 right-0 h-2 w-2 rounded-full bg-red-500">
              <span
                class="-z-1 absolute inline-flex h-full w-full animate-ping rounded-full bg-red-500 opacity-75"
              ></span>
            </span>
          ...
          >
            <div class="border-b border-stroke dark:border-dark-3 py-[18px] px-6">
              <h5 class="font-semibold text-dark dark:text-white">Notifications (02)</h5>
            </div>

            <ul class="flex h-auto flex-col gap-2 overflow-y-auto px-4 pb-20 pt-4">
              <template v-for="(item, index) in dropdownItems" :key="index">
                <li
                  class="group relative flex cursor-pointer items-center justify-between gap-4 rounded-md p-3 hover:bg-gray dark:hover:bg-dark-3"
                >
                  <a class="flex items-center gap-5" :href="item.href">
                    <div class="h-9 w-9 flex-shrink-0 rounded-full sm:h-11 sm:w-11">
                      <img
                        :src="item.image"
                        alt="User"
                    ...
                    <div>
                      <p class="mt-1 text-sm text-dark dark:text-white">
                        ...
                        {{ item.actionText }}
                        <span class="font-semibold">{{ item.textForAction }}</span>
                      </p>
                      <span class="text-sm font-medium text-primary">{{ item.time }}</span>
                    </div>
                  </a>
                  ...
                  <div class="flex items-center">
                    <div class="relative">
                      ...
                      <div
                        class="absolute right-0 top-full z-[999] mt-5 w-[180px] space-y-1 rounded border border-stroke dark:border-dark-3 bg-white dark:bg-dark-2 p-2 shadow group-last:bottom-full group-last:top-auto group-last:mb-5"

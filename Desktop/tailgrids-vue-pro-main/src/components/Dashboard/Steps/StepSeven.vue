...
const steps = ref<SingleStepProps[]>([
  { number: '1', name: 'Earings and Taxes', done: true },
  { number: '2', name: 'Review Payments' },
  { number: '3', name: 'Complete Payment' }
])
</script>

<template>
  <section class="overflow-hidden py-20 dark:bg-dark lg:py-[120px]">
        ...
        <template v-for="(step, index) in steps" :key="index">
          <div class="group flex items-center">
            <div class="mb-5 inline-flex items-center pr-4 lg:pr-0">
              <span
                class="mr-3 flex h-[34px] w-[34px] items-center justify-center rounded-full border bg-gray-2 text-base font-medium text-dark dark:bg-dark-2 dark:text-white"
                :class="{
                  'border-primary dark:bg-primary/10': step.done,
                  'border-[#E7E7E7] dark:border-dark-3': !step.done
                }"
              >
                {{ step.number }}
              </span>

              <p class="text-base text-dark dark:text-white">{{ step.name }}</p>
            </div>
            <div class="mb-5 hidden px-8 group-last:hidden lg:block text-body-color dark:text-dark-6">
              <svg
                width="28"
                height="28"
                viewBox="0 0 28 28"
